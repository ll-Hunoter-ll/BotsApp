const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am ♞HunotersBot♞. I guess you were trying to use my commands in``` *{BotsApp.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\nIf you are keen to use my features, you can deploy BotsApp on your own account without much effort, in less than 5 minutes! Check out the links given below.```\n\n🔗https://mybotsapp.com\n🔗https://github.com/ll-Hunoter-ll/BotsApp",
        ERROR_TEMPLATE:
    ...
    help: {
        ...
        HEAD: "♞ *HunotersBot Menu* ♞\n```Use .help command for detailed info on a module. Contact +48 888 888 809```",
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
        COMMAND_INTERFACE: "♞ HunotersBot Command Interface* ♞\n\n",
